...
void UMetahumanSDKAPIManager::ATLAudioToLipsync(const FMetahumanSDKATLInput& ATLInput, bool& bSuccess, FMetahumanSDKATLOutput& ATLOutput, FString& Error, FLatentActionInfo LatentInfo, UObject* WorldContextObject /*= nullptr*/)
{
    ...
    {
        bSuccess = false;
        FSystemUtilityLibrary::PrintMessage(ELogVerbosity::Error, "World is not valid!");
        return;
    }
    ...
    if (!ATLInput.Validate(Error))
    {
        ...
        FSystemUtilityLibrary::PrintMessage(ELogVerbosity::Error, FString("ATLInput isn't Valid: " + Error));
        return;
    }
...
// void UMetahumanSDKAPIManager::ATLStreamAudioToLipsync(const FMetahumanSDKATLInput& ATLInput, bool& bResponseSuccess, FMetahumanSDKATLOutput& ATLOutput, FString& Error, FLatentActionInfo LatentInfo, UObject* WorldContextObject /*= nullptr*/)
// {
//     UWorld* World = GEngine->GetWorldFromContextObjectChecked(WorldContextObject);
//     if (!IsValid(World))
//     {
...
//
//     ATLStreamAction->OnStarted.AddLambda([&bResponseSuccess](FBasePendingAction* Action, UATLStreamBuffer* Buffer, const bool bSuccessOnStarted)
//     {
//         const FATLStreamAction* CurrentAction = static_cast<FATLStreamAction*>(Action);
//         bResponseSuccess = bSuccessOnStarted;
//     });
//     
//     ATLStreamAction->OnActionCompleted.AddLambda(
//         [&bResponseSuccess, &ATLOutput, &Error, LatentInfo](FBasePendingAction* Action)
//         {
//             FATLStreamAction* CurrentAction = static_cast<FATLStreamAction*>(Action);
//             check(CurrentAction != nullptr);
//            
//             bResponseSuccess = CurrentAction->IsSuccess();
//             if (bResponseSuccess)
//             {
//                 ATLOutput.Animation = CurrentAction->OutAnimation;
//             }
...
void UMetahumanSDKAPIManager::Combo(const FMetahumanSDKComboInput& ComboInput, bool& bSuccess, FMetahumanSDKComboOutput& ComboOutput, FString& Error, FLatentActionInfo LatentInfo, UObject* WorldContextObject /*= nullptr*/)
{
    ...
    ComboAction->OnActionCompleted.AddLambda(
        [this, &bSuccess, &ComboOutput, &Error, LatentInfo](FBasePendingAction* Action)
        {
            ...
            if (bSuccess)
            {
                ...
                {                   
                    GRequestStatistics.UpdateStatistics(ComboOutput.TTSOutput.Sound->GetDuration());
                }
            }
            else
            {
