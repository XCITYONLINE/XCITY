            ...
            FResponseCompletedSignature::CreateLambda(
                [this](TSharedPtr<FGenericResponse> Response, bool bSuccess)
                {
                    if (bSuccess && Response->bResponseSuccess)
                    {
                        ...
                        const FString& ComboResponseFilename = FPaths::ConvertRelativePathToFull(FPaths::ProjectSavedDir() / TEXT("MetahumanSDK") / TEXT("Temp/ComboResponse.zip"));
                        
                        if (FFileHelper::SaveArrayToFile(ComboResponse->Data, *ComboResponseFilename))
                        {
                            const FString& ExtractDirectory = FPaths::ConvertRelativePathToFull(FPaths::ProjectSavedDir() / TEXT("MetahumanSDK") / TEXT("Temp/ComboResponse"));
