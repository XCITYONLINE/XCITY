...
UObject* UMetahumanSDKAnimSequenceAssetFactory::FactoryCreateNew(UClass* InClass, UObject* InParent, FName InName, EObjectFlags Flags, UObject* Context, FFeedbackContext* Warn)
{
	...
	ATLInput.bEyesBlinking = OptionsWindow->GetGenerateEyeMovement();
	ATLInput.bNeckMoving = OptionsWindow->GetGenerateNeckMovement();
	...
	if ( !LastATLResponse.IsValid() )
	{
		UE_LOG(LogMetahumanSDKAssetFactory, Error, TEXT("LastATLResponse Invalid!"));
		return nullptr;
	}
	else if ( !LastATLResponse->bResponseSuccess )
	{
    ...
    HttpRequest->OnProcessRequestComplete().BindLambda(
        [RequestType, StartTime, CompletedDelegate](FHttpRequestPtr Request, FHttpResponsePtr HttpResponse, bool bConnectedSuccessfully)
        {
        	...
        	if (HttpResponse == nullptr)
        	{
        		...
        		OutResponse->Error = "Http Response isn't Valid!";
        		UE_LOG(LogMetahumanSDKAssetFactory, Error, TEXT("Request error : %s"), *OutResponse->Error);
        		if (CompletedDelegate.IsBound()) CompletedDelegate.Execute(OutResponse);
        		return;
			}
            ...
            else
            {
            	...
            	FNotificationHelper::NotifyResponseError(HttpResponse->GetResponseCode(), *OutResponse->Error);
            }
...
void UMetahumanSDKAnimSequenceAssetFactory::MakeATLRequest(const FATLRequest& Request, FOnATLRequestCompleted CompletedDelegate /*= nullptr*/)
{
	...
	TWeakObjectPtr<UMetahumanSDKSettings> MetahumanSDKSettings = GetMutableDefault<UMetahumanSDKSettings>();
	if (MetahumanSDKSettings.IsValid() && MetahumanSDKSettings->APIToken.IsEmpty())
	{
		FNotificationHelper::NotifyAccessError("Fill API token via Project Settings!");
		return;
	}	
