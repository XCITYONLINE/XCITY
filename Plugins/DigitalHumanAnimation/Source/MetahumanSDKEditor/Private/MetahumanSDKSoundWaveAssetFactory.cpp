...
UObject* UMetahumanSDKSoundWaveAssetFactory::FactoryCreateNew(UClass* InClass, UObject* InParent, FName InName, EObjectFlags Flags, UObject* Context, FFeedbackContext* Warn)
{
	...
	{
		UE_LOG(LogMetahumanSDKAssetFactory, Error, TEXT("LastTtsResponse Invalid!"));		
		return nullptr;
	}
	
	if (!LastTTSResponse->bResponseSuccess)
	{
...
{
	if (GetMutableDefault<UMetahumanSDKSettings>() && GetMutableDefault<UMetahumanSDKSettings>()->APIToken.IsEmpty())
	{
