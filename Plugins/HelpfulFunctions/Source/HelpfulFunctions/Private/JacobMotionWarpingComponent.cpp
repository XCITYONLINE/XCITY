...
void UJacobMotionWarpingComponent::SetMotionWarpingTarget(FName WarpTargetName, AActor* TargetActor, FVector ConstantOffset, bool ExcludeRootMotion)
{
	...
	if (WarpKeys.Find(WarpTargetName) != -1)
	{
		...
		if (IsValid(WarpingTarget)==true)
		{ SavedWarpActorRot = WarpingTarget->GetActorRotation(); }
	}
}
